<template>
    <div>
        <h6 class="text-uppercase text-secondary fw-bolder">
            Check Availability
        </h6>
        <span v-if="noAvailability" class="text-uppercase text-secondary fw-bolder text-danger">(NOT AVAILABILITY)</span>
        <span v-if="hasAvailability" class="text-uppercase text-secondary fw-bolder text-success">(AVAILABILITY)</span>

        <div class="form row">
            <div class="form-group col-md-6">
                <label for="from">From</label>
                <input
                    type="text"
                    name="from"
                    class="form-control form-control-sm"
                    placeholder="Start data"
                    v-model="from"
                    @keyup.enter="check"
                    :class="[{'is-invalid': errorFor('from')}]"
                >
                <v-errors :errors="errorFor('from')"/>
            </div>
            <div class="form-group col-md-6">
                <label for="to">To</label>
                <input
                    type="text"
                    name="to"
                    class="form-control form-control-sm"
                    placeholder="End data"
                    v-model="to"
                    @keyup.enter="check"
                    :class="[{'is-invalid': errorFor('to')}]"
                >
                <v-errors :errors="errorFor('to')"/>
            </div>
            <button class="btn btn-secondary btn-block mt-3"
                    @click="check"
                    :disabled="loading"
            >
                Check
            </button>
        </div>
    </div>
</template>

<script>
import {is422} from "../shared/utils/response";
import validationErrors from "../shared/mixins/validationErrors";
export default {
    mixins: [
        validationErrors
    ],

    props: {
      bookableId: Number
    },

    data() {
        return {
            from: null,
            to: null,
            loading: null,
            status: null,
        }
    },

    methods: {
        check () {
            this.loading = true;
            this.errors = null;

            axios
                .post(
                    `/api/bookables/${this.bookableId}/availability`,
                    {
                        from: this.from,
                        to: this.to
                    }
                )
                .then(response => {
                    this.status = response.status;
                })
                .catch(error => {
                    if (is422(error)) {
                        this.errors = error.response.data.errors;
                    }
                    this.status = error.response.status;
                    console.log(error.response.status);

                })
                .then(() => (this.loading = false));
        },
    },

    computed: {
        hasErrors(){
            return 422 === this.status && this.errors != null;
        },
        hasAvailability(){
            return 200 === this.status
        },
        noAvailability(){
            return 404 === this.status
        },
    }
}
</script>


<style scoped>
label {
    font-size: 0.7rem;
    text-transform: uppercase;
    color: grey;
    font-weight: bolder;
}

.is-invalid {
    border-color: #b22222;
    background-image: none;
}

.invalid-feedback {
    color: #b22222;
}

</style>
